...
> {
  ...
  onMouseDown: PointerEventHandler = (event) => {
    ...
    let textArray = event.target.parentElement;
    let currentText = event.target;
    let index = 0;
    for(let i = 0; i < Array(textArray)[0].children.length; i ++ ) {
      if(Array(textArray)[0].children[i].innerText === currentText.innerText) {
        index = i;
        break;
      }
    }
    let targetText = '';
    for(let j = index; j < Array(textArray)[0].children.length; j ++) {
      targetText += ' ';
      targetText += Array(textArray)[0].children[j].innerText;
    }

    console.error(targetText);
    this.props.onGetText(targetText);
  ...
  afterSelection = () => {
    ...
    // this.props.onGetText(content.text);
